...
describe('Student Registration form Test in desktop viewport size', () => {
   ...
   it('Test Case 1: Verify the form allows successfully registration of an student with only required fields', () => {
    ...
    it('Test Case 2: Verify the form allows successfully registration of an student with all theinformation', () =>  { 
      ...
      cy.get('#lastName').type('Karen')
      cy.get('#userEmail').type('karentest@mail.com')
      cy.getOption('[type="radio"]')
      ...
      cy.get('#currentAddress').type('6750 W Olive Ave, 85345')
      cy.get('#state .css-tlfecz-indicatorContainer').click({ force: true })
      cy.get('#react-select-3-option-1').click({ force: true })
      cy.get('#city .css-tlfecz-indicatorContainer').click({ force: true })
      cy.get('#react-select-4-option-0').click({ force: true })
      cy.get('#submit').click({ force: true })
      
      cy.get('.modal-content').should('be.visible')
      cy.get('#example-modal-sizes-title-lg').should('be.visible').contains('Thanks for submitting the form')
      cy.get('tbody tr:nth-of-type(1) td:nth-of-type(2)').should('be.visible').contains('Test Karen')
      cy.get('tbody tr:nth-of-type(4) td:nth-of-type(2)').should('be.visible').contains('5441254784')
